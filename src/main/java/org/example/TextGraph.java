...
public class TextGraph {
    private final Map<String, Set<Edge>> graph;  //一个映射，也就是一个字符串映射到一个edge类的集合，该edge类有两个属性，顶点名字以及该条边的权重，如下
    ...
    void showDirectedGraph(Map<String, Set<Edge>> graph1, String... shortestPath) throws IOException {
        Graph<String, DefaultWeightedEdge> graph2 = new DefaultDirectedWeightedGraph<>(DefaultWeightedEdge.class);
        graph1.keySet().forEach(graph2::addVertex);   // 构造顶点
        for (Map.Entry<String, Set<Edge>> entry : graph1.entrySet()) { //构造边
            String key = entry.getKey();
            if (!entry.getValue().isEmpty()) {
                for (Edge edge : entry.getValue()) {
                    DefaultWeightedEdge edge1 = graph2.addEdge(key, edge.vertex);
                    if (edge1 != null) {
                        graph2.setEdgeWeight(edge1, edge.weight);
                    }
                }
            }
        }
        JGraphXAdapter<String, DefaultWeightedEdge> graphAdapter = new JGraphXAdapter<>(graph2);
        graphAdapter.setEdgeLabelsMovable(true);
        for (DefaultWeightedEdge edge : graph2.edgeSet()) {
            String weightLabel = String.valueOf(graph2.getEdgeWeight(edge));
        ...
        if (shortestPath.length > 0) {
            ...
            for (int i = 0; i < pathEdges.length - 1; i++) {
                ...
                DefaultWeightedEdge edge = graph2.getEdge(source, target);
                if (edge != null) {
            ...
            JLabel label = new JLabel(new ImageIcon(image)); // 创建一个JLabel来显示图片
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        textGraph.readFile(filePath, false);
        Scanner scanner = new Scanner(System.in, StandardCharsets.UTF_8);
        ...
        while (true) {
            ...
            switch (choice) {
                case 1 -> textGraph.showDirectedGraph(textGraph.graph);
                case 2 -> {
                ...
                case 4 -> {
                    ...
                    if (!startWord.isEmpty() && endWord.isEmpty()) {
                                ...
                                if (!vertex.equals(startWord)) {
                                    ...
                                    if (!shortestPath.startsWith("No")) {
                                        ...
                                        System.out.println("The length of the shortest path from \""
                                                + sourceWord + "\" to \"" + vertex + "\" is: " + pathLength);
                                    }
                                }
                            }
                        }
                            ...
                            for (String vertex : textGraph.graph.keySet()) {
                                if (!vertex.equals(endWord)) {
                                    ...
                                    if (!shortestPath.startsWith("No")) {
                                        ...
                                        System.out.println("The length of the shortest path from \""
                                                + targetWord + "\" to \"" + vertex + "\" is: " + pathLength);
                                    }
                                }
                            }
                        }
                        ...
                        if (!shortestPath.startsWith("No")) {
                            ...
                            System.out.println("The length of the shortest path from \""
                                    + sourceWord + "\" to \"" + targetWord + "\" is: " + pathLength);
                        }
                    }
                }
                case 5 -> {
                    ...
                    try {
                        BufferedWriter writer = new BufferedWriter(
                                new FileWriter(
                                        "src/main/java/org/example/random_walk_path.txt", StandardCharsets.UTF_8));
